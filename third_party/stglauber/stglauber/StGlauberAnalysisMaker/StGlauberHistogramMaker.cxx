...
void StGlauberHistogramMaker::SetTableDirectory(const TString directory)
{
  ...
  std::cout << "StGlauberHistogramMaker::SetTableDirectory Set output directory for table : "
       << mTableDirectory.Data() << std::endl;
}
...
void StGlauberHistogramMaker::Init()
{
  ...
  for(vector<TProfile*>::iterator iter = mProfile.begin();
    ...
    std::cout << Form("StGlauberHistogramMaker::Init  Initialize TProfile: %s (%s), x:(bin, min, max) = (%4d, %1.2f, %1.2f)", 
...
void StGlauberHistogramMaker::SetXaxis(const StGlauberTree& tree, const StCentralityMaker& centralityMaker, const TString type)
{
  ...
  if(mDebug){
    std::cout << Form("StGlauberHistogramMaker::SetXaxis  mult=%6d, centrality: (default, -5%, +5%) = (%1.1f, %1.1f, %1.1f)",
...
void StGlauberHistogramMaker::DoWeightCorrection(vector<TH1*> collection1d, vector<TProfile*> collectionp)
{
  ...
  std::cout << Form("StGlauberHistogramMaker::DoWeightCorrection %s: particle-wise weight correction, profile/weight",
      mName.Data()) << std::endl;
...
void StGlauberHistogramMaker::WriteTable(vector<TH1*> collection, const TString name)
{
  ...
  std::cout << "StGlauberHistogramMaker::WriteTable  Write table " <<  tableName.Data() << std::endl;
...
void StGlauberHistogramMaker::WriteGraphs(vector<TH1*> collection)
{
  ...
  std::cout << Form("StGlauberHistogramMaker::WriteGraphs  Write graph %s (vs centrality)", graph->GetName()) << std::endl;
...
{
  std::cout << "StGlauberHistogramMaker::Finish  ";
  std::cout << Form("Finish %s. Correction weight for TProfile, ", mName.Data()); 
  std::cout << "and store it into 1D. Write averate qunatity vs centrality" << std::endl;
...
{
  mDebug = 1 ;
  std::cout << "StGlauberHistogramMaker::DebugOn  Print debug messages" << std::endl;
}
