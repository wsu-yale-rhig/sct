...
Int_t StGlauberPlotMaker::Read(const TString filename, const TString type)
{
  ...
  std::cout << Form("StGlauberPlotMaker::Read  OPEN %s (%s)", filename.Data(), type.Data()) << std::endl;
  ...
  std::cout << "StGlauberPlotMaker::Read  Init graphs: "
  ...
  for(UInt_t ic=0; ic<StGlauberConstUtilities::GetCentralityBin(); ic++){
    ...
    std::cout << "StGlauberPlotMaker::Read  " << mName << ": centrality (min,max) = ("
         << centMin << "," << centMax << "), val = "
         << val << " +/- " << error
         << std::endl;
...
TGraphErrors* StGlauberPlotMaker::SystematicErrors(const UInt_t mode)
{
  ...
  std::cout << "StGlauberPlotMaker::SystematicErrors  Start evaluating systematic errors" << std::endl;
  ...
  for(UInt_t id=1; id<nSize; id++){
    std::cout << "StGlauberPlotMaker::SystematicErrors Use " << mGraph[id]->GetTitle() << std::endl;
...
//  c1->Print(Form("%s_systematic_error.png", mName.Data()));
  ...
  std::cout << Form("StGlauberPlotMaker::Draw  Write table %s in the current directory", tableName.Data()) << std::endl;
