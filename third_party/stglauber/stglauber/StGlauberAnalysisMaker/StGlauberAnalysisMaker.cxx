...
#include <iostream>
#include "stglauber/StCentralityMaker/StCentrality.h"
...
//    std::cout << Form("StGlauberAnalysisMaker  Define centrality id map: id=%5d for type %20s",
...
Bool_t StGlauberAnalysisMaker::Init(const TString tableDir)
{
  ...
  std::cout << "StGlauberAnalysisMaker::Init  Open output ROOT file: " << mOutputFile->GetName() << std::endl;
...
Bool_t StGlauberAnalysisMaker::Make()
{
  /// Set x-axis
  if(mDebug){
    std::cout << "StGlauberAnalysisMaker::Make  Set x-axis" << std::endl;
  }
...
//    mEccPPM[io]->SetXaxis(*mGlauberTree, *(mCentralityMaker->GetCentrality(centId)), mType);
  ...
  if(mDebug){
    std::cout << "StGlauberAnalysisMaker::Make  Start fill: " << std::endl;
  }
  ...
  if(mDebug){
    std::cout << "StGlauberAnalysisMaker::Make  reweighting = " << reweighting
...
//  std::cout << areaRP << " " << areaPP << std::endl;
...
Bool_t StGlauberAnalysisMaker::RunFile(const TString inputFileName)
{
  ...
  std::cout << Form("StGlauberAnalysisMaker::RunFile  Process %10d events ...", nevents) << std::endl;

  for(Int_t ievent=0; ievent<nevents; ievent++){
    mGlauberTree->Clear() ;

    if(mDebug){
      std::cout << "StGlauberAnalysisMaker::RunFile  Event# = " << ievent << std::endl;
    }
    ...
    /// Debugging message
    if( ievent%1000 == 0 ){
      std::cout << "StGlauberAnalysisMaker::RunFile  ";
      std::cout << Form("event %10d/%10d : (Npart, Ncoll, b(fm), mult, cent, {x^2}, {x}) = (%5d, %5d, %1.2f, %5d, %1.1f, %1.4f, %1.4f)",
  ...
  /// Clean up
  if(mDebug){
    std::cout << "StGlauberAnalysisMaker::RunFile  Close input file" << std::endl;
  }
  mGlauberTree->Close() ;
...
Bool_t StGlauberAnalysisMaker::Run(const TString inputFileList)
{
  ...
  std::cout << "StGlauberAnalysisMaker::Run  Open file list: " << inputFileList.Data() << std::endl;
...
Bool_t StGlauberAnalysisMaker::Finish()
{
  ...
  std::cout << "StGlauberAnalysisMaker::Finish  Write/Close output file: " << mOutputFile->GetName() << std::endl;
  mOutputFile->Write();
  mOutputFile->Close();
...
{
  mUnitWeight = kTRUE ;
  std::cout << "StGlauberAnalysisMaker::UnitWeightOn  Use unit weight" << std::endl ;
}
...
{
  mReweighting = kTRUE ;
  std::cout << "StGlauberAnalysisMaker::ReweightingOn  Apply re-weighting correction" << std::endl;
}
...
void StGlauberAnalysisMaker::DebugOn()
{
  ...
  std::cout << "StGlauberAnalysisMaker::DebugOn  Print debugging messages" << std::endl;
}
