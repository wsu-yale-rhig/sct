...
void StGlauberCumulantHistogramMaker::Init()
{
  ...
  for(UInt_t io=0; io<mNOrder; io++){
    ...
    for(vector<TProfile*>::iterator iter = mProfileCumulant[io].begin();
      ...
      std::cout << Form("StGlauberCumulantHistogramMaker::Init Initialize TProfile: %s (%s), x:(bin, min, max) = (%4d, %1.2f, %1.2f)", 
...
void StGlauberCumulantHistogramMaker::Finish(const TString type)
{
  ...
  /// Calculate cumulant
  std::cout << "StGlauberCumulantHistogramMaker::Finish Calculate cumulant" << std::endl;
