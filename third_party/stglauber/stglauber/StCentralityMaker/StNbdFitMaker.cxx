...
StNbdFitMaker::StNbdFitMaker() {
  ...
  mMinimumMultiplicityCut = 100.0; // >50 by default. Can be changed by
                                   // setMinimumMultiplicityCut(const Double_t)
...
void StNbdFitMaker::DoCentralityDetermination() {
  std::cout << "StNbdFitMaker::DoCentralityDetermination  Centrality "
               "determination is ON"
            << std::endl;
  mDoCentralityDetermination = kTRUE;
}
...
//____________________________________________________________________________________________________
Double_t StNbdFitMaker::GetNormalization(const TH1 &h1, const TH1 &h2,
                                         const Double_t min,
  ...
  std::cout << Form("StNbdFitMaker::GetNormalization  (min, max, norm) = (%5d, "
                    "%5d, %1.7f)",
...
//____________________________________________________________________________________________________
Double_t StNbdFitMaker::CalculateChi2(const TH1 &hdata, const TH1 &hfunc,
  ...
  std::cout << Form("StNbdFitMaker::calculateChi2  M > %5d: (npp, k) = (%1.5f, "
                    "%1.5f)  chi2/ndata = %1.3f/%5d = %1.3f",
                    static_cast<Int_t>(minimumMultiplicityCut),
                    mNBinomial->GetNpp(), mNBinomial->GetK(), chi2, mNData,
                    chi2 / static_cast<Double_t>(mNData))
            << std::endl;
...
//____________________________________________________________________________________________________
void StNbdFitMaker::CalculateCentrality(const TH1 &hdata,
                                        const TH1 &hmc) const {
  // - Calculate centrality from the MC multiplicity distribution
  // - Also calculate the re-weighting correction = MC/Data, important for
  // peripheral (typically 60-80%)
  //
  ...
  for (UInt_t i = 0; i < 2; i++) {
    ...
    if (i == 0)
      std::cout << "StNbdFitMaker::CalculateCentrality  (1) Centrality "
                   "determination from peripheral to central"
                << std::endl;
    if (i == 1)
      std::cout << "StNbdFitMaker::CalculateCentrality  (2) Centrality "
                   "determination from central to peripheral"
                << std::endl;
    ...
    for (UInt_t ic = 0; ic < ncent; ic++) {
      ...
      centralityMax[ic] =
          (centMinStart + centStep * 100.0) + sign * centStep * 100.0 * ic;

      // Print centrality definitions
      std::cout << Form("StNbdFitMaker::CalculateCentrality  Centrality: "
                        "%1.0f-%1.0f %%, cut = %1.2f",
                        TMath::Abs(centralityMin[ic]),
                        TMath::Abs(centralityMax[ic]), centralityCut[ic])
                << std::endl;
      ;
    }
    ...
    for (Int_t it = 0; it < 3;
         it++) { // vary total cross section by 5 % in it=1,2
      Double_t scale = 1.0;
      if (it == 1)
        scale = 0.95;
      if (it == 2)
      ...
      for (Int_t im = 0; im < nbin; im++) {
        const Double_t M =
            (i == 0) ? hmc.GetBinCenter(im + 1) : hmc.GetBinCenter(nbin - im);
        const Int_t Mint = (i == 0) ? im : nbin - im - 1;
        const Double_t count =
            (i == 0) ? hmc.GetBinContent(im + 1) : hmc.GetBinContent(nbin - im);
        if (count == 0.0)
          continue;
        ...
        if (isCentOk && bin < ncent) {
          std::cout << Form(
              "%2.2f - %2.2f (%%) :  M > %4d (im=%3d, M=%1.1f, bin=%4d) (sum, "
              "total, fraction>cut) = (%1.3f, %1.3f, %1.3f>%1.3f)",
              TMath::Abs(centralityMin[bin] * scale),
              TMath::Abs(centralityMax[bin] * scale), Mint, im, M, bin, sum,
              nevent, R, fCentBinCut) << std::endl;
  ...
  // Print centrality bins in the array format for implementation in
  // StCentrality.cxx
  // - Use central to peripheral
  for (UInt_t ic = 0; ic < ncent; ic++) {
    std::cout << Form(
        "  mMultiplicityCut[0].push_back( %3d ); mCentralityMin[0].push_back( "
        "%2.1f ); mCentralityMax[0].push_back( %2.1f );",
        (Int_t)centBin[1][0][ic], 5.0 * ic, 5.0 * (ic + 1)) << std::endl;
  }

  // For +/- 5% bins
  for (UInt_t ic = 0; ic < ncent; ic++) {
    std::cout << Form("  mMultiplicityCut[1].push_back( %3d );  "
                      "mMultiplicityCut[2].push_back( %3d );",
                      (Int_t)centBin[1][1][ic], (Int_t)centBin[1][2][ic]) << std::endl;
  }
}

//____________________________________________________________________________________________________
void StNbdFitMaker::SetParameters(const Double_t npp, const Double_t k,
                                  const Double_t x, const Double_t efficiency,
                                  const Double_t triggerbias,
                                  const Bool_t isConstEfficiency) {
  if (mNBinomial)
    delete mNBinomial;

  mNBinomial = new StNegativeBinomial(npp, k, x, efficiency, triggerbias,
                                      isConstEfficiency);
}
...
void StNbdFitMaker::SetMinimumMultiplicityCut(const Double_t cut) {
  ...
  std::cout << "StNbdFitMaker::setMinimumMultiplicityCut  Set low multiplicity "
               "cut off : M > "
            << cut << std::endl;
}

//____________________________________________________________________________________________________
void StNbdFitMaker::ReadData(const Char_t *data, const Char_t *glauber,
                             const Char_t *dataHistogramName) {
  // Read real data file
  ...
  std::cout << "StNbdFitMaker::readData  open Data file: "
            << inputData->GetName() << std::endl;;
  ...
  std::cout << "StNbdFitMaker::readData  open Glauber file: "
            << inputGlauber->GetName() << std::endl;
...
//____________________________________________________________________________________________________
TGraph *StNbdFitMaker::Fit(const Int_t nevents, const Char_t *outputFileName) {
  gStyle->SetOptStat(0);
  ...
  while (ievent < nevents) {
    ...
    const Int_t multiplicity =
        mNBinomial->GetMultiplicity(npart, static_cast<Int_t>(ncoll));
    mhRefMultSim->Fill(multiplicity);

    if (ievent % (nevents / 10) == 0) {
      std::cout << Form("StNbdFitMaker::Fit  ievent=%10d  (npart, ncoll, mult) "
                        "= (%10d, %10d, %10d)",
                        ievent, (Int_t)npart, (Int_t)ncoll, multiplicity) << std::endl;
    }
  ...
  // Normalization
  const Double_t norm =
      GetNormalization(*mhRefMult, *mhRefMultSim, mMinimumMultiplicityCut,
                       mhRefMult->GetXaxis()->GetXmax());
  // const Double_t norm = GetNormalization(*mhRefMult, *mhRefMultSim,
  // mMinimumMultiplicityCut, 400);
  mhRefMultSim->Scale(norm);

  // Get chi2
  const Double_t chi2 =
      CalculateChi2(*mhRefMult, *mhRefMultSim, mMinimumMultiplicityCut);
  ...
  mhRefMultSim->SetTitle(Form("npp=%1.2f, k=%1.2f, x=%1.2f",
                              mNBinomial->GetNpp(), mNBinomial->GetK(),
                              mNBinomial->GetX()));
  ...
  if (mCutOff[0])
    delete mCutOff[0];
  mCutOff[0] = new TLine(mMinimumMultiplicityCut, mhRefMult->GetMinimum(),
                         mMinimumMultiplicityCut, mhRefMult->GetMaximum());
  ...
  if (mOneLine)
    delete mOneLine;
  mOneLine = new TLine(hRatio->GetXaxis()->GetXmin(), 1.0,
  ...
  if (mCutOff[1])
    delete mCutOff[1];
  mCutOff[1] = new TLine(mMinimumMultiplicityCut, hRatio->GetMinimum(),
                         mMinimumMultiplicityCut, hRatio->GetMaximum());
  ...
  if (!fileName.IsWhitespace()) {
    std::cout << "StNbdFitMaker::Fit  Write output ROOT file : "
              << outputFileName;
...
//____________________________________________________________________________________________________
Int_t StNbdFitMaker::Scan(
    const Int_t nevents, const Int_t nppbin, const Double_t nppmin,
  ...
  TH3 *hChi2 = new TH3D("hChi2", "#chi^{2}/NDF in (npp, k, x) space", nppbin,
                        nppmin, nppmax, kbin, kmin, kmax, xbin, xmin, xmax);
  ...
  for (Int_t ix = 0; ix < nppbin; ix++) {
    ...
    for (Int_t iy = 0; iy < kbin; iy++) {
      ...
      std::cout << Form(
          "StNbdFitMaker::Scan  Fitting for (npp, k) = (%1.3f, %1.3f) ...", npp,
          k) << std::endl;
      ...
      for (Int_t iz = 0; iz < xbin; iz++) {
        ...
        const Char_t *fileforratio(
            Form("Ratio_npp%1.3f_k%1.3f_x%1.3f_eff%1.3f.root", npp, k, x,
                 efficiency));
        ...
        std::cout << Form(
            "StNbdFitMaker::Scan  Fitting for (npp, k, x, chi2/ndf) = (%1.3f, "
            "%1.3f, %1.3f, %1.3f/%3d=%1.3f) ...",
            npp, k, x, mChi2Graph->GetY()[0], (Int_t)mChi2Graph->GetY()[1],
            mChi2Graph->GetY()[2]) << std::endl;
      } // x loop
    }   // k loop
  }     // npp loop
  ...
  //----------------------------------------------------------------------------------------------------
  const Char_t *fileName(Form(
      "chi2_nevents%d_npp%1.3f-%1.3f_k%1.3f-%1.3f_x%1.3f_%1.3f_eff%1.3f.root",
