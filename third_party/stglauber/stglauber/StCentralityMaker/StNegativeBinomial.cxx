...
{
  std::cout  << Form("StNegativeBinomial  (npp, k, x, eff, trig. bias) = (%1.3f, %1.3f, %1.3f, %1.3f, %1.3f)",
        mNpp, mK, mX, mEfficiency, mTriggerBias) << std::endl;

  if(mIsConstEfficiency){
    std::cout  << "StNegativeBinomial  Use constant efficiency" << std::endl;
  }
  else{
    std::cout  << "StNegativeBinomial  Use multiplicity dependent efficiency" << std::endl;
  }
...
Int_t StNegativeBinomial::GetMultiplicity(const Double_t npart, const Double_t ncoll) const
{
  ...
  //cout<<efficiency<<" << std::endl;"<<npart<<" << std::endl;"<<ncoll<<"   TEST "<<endl;
