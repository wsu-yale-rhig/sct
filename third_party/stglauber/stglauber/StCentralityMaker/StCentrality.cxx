  ...
  : mType(type), mNpp(0.0), mK(0.0), mX(0.0)
{
  ...
  for(UInt_t im=0; im<mNMode; im++){
    for(UInt_t ic=0; ic<mMultiplicityCut[im].size(); ic++){
      std::cout  << Form("StCentrality  multiplicity cut M > %4d for centrality %3d - %3d (%%)", 
...
{
  std::cout  << "#----------------------------------------------------------------------------------------------------" << std::endl;
  std::cout  << "StCentrality::help  Current available systems for Init(const Char_t* system) are:" << std::endl;
  std::cout  << "system         descriptions" << std::endl;
  std::cout  << "AuAu_200GeV    Au + Au at 200 GeV" << std::endl;
  std::cout  << "AuAu_39GeV     Au + Au at 39 GeV" << std::endl;
  std::cout  << "AuAu_11GeV     Au + Au at 11.5 GeV" << std::endl;
  std::cout  << "AuAu_19GeV     Au + Au at 19.6 GeV" << std::endl;
  std::cout  << "AuAu_7GeV      Au + Au at 7.7 GeV" << std::endl;
  std::cout  << "dAu_200GeV      d + Au at 200 GeV" << std::endl;
  std::cout  ;
  std::cout  << "  NOTE: input system name is case insensitive" << std::endl;
  std::cout  << "#----------------------------------------------------------------------------------------------------" << std::endl;
}
...
Double_t StCentrality::GetNppError(const Double_t npp) const
{
  if ( mType.CompareTo("default", TString::kIgnoreCase) == 0 )       return 0.0 ;
    ...
    std::cout  << "StCentrality::GetNppError  type=low  add -5% error on npp" << std::endl;
    return -0.05 * npp ;
  }
  else if ( mType.CompareTo("high", TString::kIgnoreCase) == 0 ){
    std::cout  << "StCentrality::GetNppError  type=high  add +5% error on npp" << std::endl;
    return 0.05 * npp ;
  }
  else {
...
void StCentrality::Init_AuAu39GeV()
{
	...
	std::cout  << "StCentrality::Init_AuAu39GeV  Use " << mType << " type: re-weighting correction will be "
