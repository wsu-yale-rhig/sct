...
namespace sct {
  ...
  bool GlauberSystematics::run() {
    ...
    for (auto& var : variations_) {
      ...
      for (int i = 0; i < reader.getEntries(); ++i) {
        ...
        event_dict[GlauberObservable::Multiplicity] = reader.multiplicity();
        event_dict[GlauberObservable::Centrality] = getCentrality(reader.multiplicity());
  ...
  bool GlauberSystematics::initialize() {
