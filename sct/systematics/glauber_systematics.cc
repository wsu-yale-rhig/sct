...
namespace sct {
  ...
  GlauberSystematics::GlauberSystematics(const string& out_filename) :
   out_filename_(out_filename), do_npp_variations_(false), use_unit_weight_(false) { 
  ...
  bool GlauberSystematics::load(const string& glauber_filename) {
    ...
    try {
      ...
      LOG(ERROR) << "Could not identify glauber systematic variation from filename: add tag manually with "
                 << "Load(glauber_file, tag). Loading failed for " << glauber_filename;
      return false;
    }
  ...
  void GlauberSystematics::setMultiplicityModel(double npp, double k, double x, double ppEff,
                                                ...
                                                bool constEff) {
    mult_model_[GlauberMod::Nominal] = make_unique<MultiplicityModel>(npp, k, x, ppEff, aaEff, aaCent, trigEff, constEff);
    mult_model_[GlauberMod::LargeNpp] = make_unique<MultiplicityModel>(npp*1.05, k, x, ppEff, aaEff, aaCent, trigEff, constEff);
    mult_model_[GlauberMod::SmallNpp] = make_unique<MultiplicityModel>(npp*0.95, k, x, ppEff, aaEff, aaCent, trigEff, constEff);
  }
  ...
  void GlauberSystematics::setCentralityBins(std::vector<double> cent_def) {
    ...
    std::sort(centrality_lower_bounds_.begin(), centrality_lower_bounds_.end(), std::greater<double>());
  }
  ...
  bool GlauberSystematics::write() {
    ...
    for (auto& hist : pp_ecc_)
      hist.write(out_file_.get());
    for (auto& hist : pp_ecc_mult_)
      hist.write(out_file_.get());
  ...
  void GlauberSystematics::initialize() {
    ...
    for (auto& container : pp_ecc_)
      container.calculateCumulants();
    for (auto& container : pp_ecc_mult_)
      container.calculateCumulants();
    ...
    for (auto& var : variations_) {
      ...
      for (auto& entry : pp_ecc_)
        entry.add(key);
      for (auto& entry : pp_ecc_mult_)
        entry.add(key);
    }
  }
  ...
  bool GlauberSystematics::loadFile(const string& filename, GlauberMod tag) {
    ...
    if (file.get() == nullptr ||  !file->IsOpen() || 
      ...
      LOG(ERROR) << "Could not open glauber file: " << filename << ": file loading failed";
      return false;
    }

    if (variations_.find(tag) != variations_.end()) {
      LOG(WARNING) << "variation " << glauberModToString[tag] << " already exists as key to "
                   << variations_[tag]->GetFile() << ", overwriting this with filename";
    }
  ...
  GlauberMod GlauberSystematics::findFileTag(const string& filename) {
      ...
      if (findCaseInsensitive(filename, mod)) {
        if (!tag.empty()) {
          LOG(WARNING) << "warning - filename: " << filename 
                     ...
                     << " multiple possible variations. Overwriting "
                     << tag << " with " << mod;
        }
        tag = mod;
      }
    ...
    if (tag.empty())
      throw std::invalid_argument(MakeString("Could not identify a Glauber Modification from filename ", filename));
  ...
  int GlauberSystematics::getCentrality(double multiplicity) {
    for (int i = 0; i < centrality_lower_bounds_.size(); ++i) {
      if (multiplicity > centrality_lower_bounds_[i])
        return i;
    }
  ...
  void GlauberSystematics::runFile(GlauberMod key, shared_ptr<TFile> file) {
    ...
    for (int i = 0; i < reader.getEntries(); ++i) {
      ...
      for (auto& hist : pp_ecc_)
        hist.fillEvent(key, event_dict, weight);

      // fill event dict with multiplicity weighted values
      event_dict[GlauberObservable::RP2Ecc] = reader.RP2Ecc(GlauberWeight::Multiplicity);
      event_dict[GlauberObservable::PP2Ecc] = reader.PP2Ecc(GlauberWeight::Multiplicity);
      event_dict[GlauberObservable::PP3Ecc] = reader.PP3Ecc(GlauberWeight::Multiplicity);
      event_dict[GlauberObservable::PP4Ecc] = reader.PP4Ecc(GlauberWeight::Multiplicity);
