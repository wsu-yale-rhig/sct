...
SystematicVariable::SystematicVariable()
    : y_(),
      name_(""),
      label_(""),
      bins_(0),
...
void SystematicVariable::add(GlauberMod mod) {
  ...
  // SystematicVariable does not support multiple copies of the same
  // modification
  ...
  for (auto& obs : x_axis_variables_) {
    ...
    if (cumulant_flag_) {
      for (auto& order : cumulant_order_) {
        string name_1d_moment =
            name + histTypeString[HistType::MomentTH1] + std::to_string(order);
        string name_prof_moment =
            name + histTypeString[HistType::MomentProf] + std::to_string(order);
        string name_1d_cumulant =
            name + histTypeString[HistType::Cumulant] + std::to_string(order);

        string title_cumulant =
            MakeString(";", histInfo.label(obs), ";", name_);
...
bool SystematicVariable::fillEvent(GlauberMod mod, EventDict<double>& dict,
                                   double weight) {
  ...
  for (auto& x : x_axis_variables_) {
    ...
    if (cumulant_flag_) {
      for (auto& order : cumulant_order_) {
        // get historam names
        string name_moment =
            getHistogramName(mod, x, HistType::MomentProf, order);
...
void SystematicVariable::write(TFile* file) {
  ...
  // start to write out, filling in the TH1Ds with proper weighted results as we
  // go
  for (auto& mod : modifications_) {
    for (auto& x : x_axis_variables_) {
      ...
      TProfile* tmp_weight =
          weights_.get(getHistogramName(mod, x, HistType::Weight));
      ...
      if (cumulant_flag_ && cumulant_order_.size() > 0) {
        ...
        std::vector<TH1D*> corrected_moments(*cumulant_order_.rbegin() + 1,
                                             nullptr);
        std::vector<TH1D*> cumulants(*cumulant_order_.rbegin() + 1, nullptr);
        for (auto& order : cumulant_order_) {
          TProfile* tmp_moment_p =
              tprof_.get(getHistogramName(mod, x, HistType::MomentProf, order));
          TH1D* tmp_moment_1d =
              th1_.get(getHistogramName(mod, x, HistType::MomentTH1, order));
          TH1D* tmp_cumulant_1d =
              th1_.get(getHistogramName(mod, x, HistType::Cumulant, order));
        ...
        for (int i = 1;
             i <= corrected_moments[*cumulant_order_.begin()]->GetNbinsX();
             ++i) {
          // we make vectors of the moments/errors. we need even moments up to
          // the maximum value specified in cumulant_order_. This is a std::set
          // so the maximum value is always stored last, so we get it with
          // rbegin()
          std::vector<double> moments(*cumulant_order_.rbegin() + 1, 0);
          std::vector<double> moment_errors(*cumulant_order_.rbegin() + 1, 0);

          // now each cumulant only depends on moments of the same order or
          // less, so we can collect the moments and calculate the cumulants in
          // the same loop
          for (auto& order : cumulant_order_) {
            ...
            double cumulant_error =
                nthOrderCumulantError(moments, moment_errors, order);
...
TH1* SystematicVariable::get(GlauberMod mod, GlauberObservable x_axis,
                             HistType tag, unsigned cumulant_order) {
  string hist_name = getHistogramName(mod, x_axis, tag, cumulant_order);

  if (tag == HistType::TH1) return th1_.get(hist_name);
  if (tag == HistType::TH2) return th2_.get(hist_name);
  if (tag == HistType::Prof) return tprof_.get(hist_name);
  if (tag == HistType::Weight) return weights_.get(hist_name);

  if (cumulant_flag_) {
    for (auto& order : cumulant_order_) {
      string moment_tag_1d =
          histTypeString[HistType::MomentTH1] + std::to_string(order);
      string moment_tag_prof =
          histTypeString[HistType::MomentProf] + std::to_string(order);
      string cumulant_tag =
          histTypeString[HistType::Cumulant] + std::to_string(order);
...
string SystematicVariable::getHistogramName(GlauberMod mod,
                                            GlauberObservable x_axis,
                                            HistType tag,
                                            unsigned cumulant_order) {
...
void SystematicVariable::reweight(TProfile* source, TProfile* weight,
                                  TH1D* target) {
...
double SystematicVariable::nthOrderCumulant(std::vector<double>& moments,
                                            unsigned order) {
  ...
  if (moments.size() < order) {
    LOG(ERROR) << "moments vector: " << moments
               << " does not contain enough moments "
               << "to be calculating the " << order
               << " cumulant. Must have at least " << order << " moments.";
    return failure;
  }
  ...
  switch (order) {
    ...
    case 6:
      ...
      mu_4 = moments[4];
      mu_6 = moments[6];
      return pow(abs(0.25 * (mu_6 - 9.0 * mu_4 * mu_2 + 12.0 * pow(mu_2, 3))),
                 1.0 / 6.0);
...
double SystematicVariable::nthOrderCumulantError(std::vector<double>& moments,
  ...
  if (moments.size() < order) {
    LOG(ERROR) << "moments vector: " << errors
               << " does not contain enough errors "
               << "to be calculating the " << order
               << " cumulant. Must have at least " << order << " errors.";
    return failure;
  }
    ...
    case 6:
      ...
      error_6_1 =
          mu_4 == 0.0 || mu_2 == 0.0
              ? 0.0
              : 9.0 * abs(mu_4 * mu_2) *
                    sqrt(pow(mu_4_err / mu_4, 2.0) + pow(mu_2_err / mu_2, 2.0));
      error_6_2 = 12.0 * 3.0 * mu_2 * mu_2 * mu_2_err;

      cumulant_6 = nthOrderCumulant(moments, 6);
      error_6 = 0.25 * sqrt(pow(mu_6_err, 2.0) + pow(error_6_1, 2.0) +
                            pow(error_6_2, 2.0));
