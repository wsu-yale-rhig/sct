...
TEST(SystematicVariable, initialization) {
  std::set<sct::GlauberMod> mods{
    sct::GlauberMod::Nominal, 
    sct::GlauberMod::Large,
    sct::GlauberMod::SmallXSec
    };
  std::set<sct::GlauberObservable> x_axis_observables{
    sct::GlauberObservable::B, 
    sct::GlauberObservable::Ncoll, 
    sct::GlauberObservable::Npart, 
    sct::GlauberObservable::Multiplicity
    }; 
...
TEST(SystematicVariable, cumulantInitialization) {
  ...
  std::set<sct::HistType> cumulant_types {
    sct::HistType::Cumulant,
    sct::HistType::MomentProf,
    sct::HistType::MomentTH1
  };
...
TEST(SystematicVariable, filling) {
  ...
  for (auto& x_axis : x_axis_observables) {
      for (auto& mod : mods) {
        ...
        int nbins_y = sct::HistogramInfo::instance().bins(sct::GlauberObservable::Centrality);
        double low_y = sct::HistogramInfo::instance().lowEdge(sct::GlauberObservable::Centrality);
        double high_y = sct::HistogramInfo::instance().highEdge(sct::GlauberObservable::Centrality);
        ...
        if (mod == sct::GlauberMod::Nominal) {
          ...
          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::Prof)->Integral(), nominalweight*event[sct::GlauberObservable::Centrality]);
          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::Weight)->Integral(), nominalweight);
          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::TH2)->Integral(), 1.0);

          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::Prof)->GetBinContent(bin_x), nominalweight*event[sct::GlauberObservable::Centrality]);
          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::Weight)->GetBinContent(bin_x), nominalweight);

          int bin_2d = var.get(mod, x_axis, sct::HistType::TH2)->GetBin(bin_x, bin_y);
          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::TH2)->GetBinContent(bin_2d), 1.0);

          // test moments
          for (auto& order : {2, 4, 6}) {
            EXPECT_EQ(var.get(mod, x_axis, sct::HistType::MomentTH1, order)->Integral(), 0.0);
            EXPECT_EQ(var.get(mod, x_axis, sct::HistType::MomentProf, order)->Integral(), 
                      nominalweight*pow(event[sct::GlauberObservable::Centrality], order));
            EXPECT_EQ(var.get(mod, x_axis, sct::HistType::MomentProf, order)->GetBinContent(bin_x), 
                      nominalweight*pow(event[sct::GlauberObservable::Centrality], order));
            EXPECT_EQ(var.get(mod, x_axis, sct::HistType::Cumulant, order)->Integral(), 0.0);
          }
        }
        
        else if (mod == sct::GlauberMod::Large) {
          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::TH1)->Integral(), 0.0);
          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::Prof)->Integral(),largeweight*event[sct::GlauberObservable::Centrality]);
          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::Weight)->Integral(), largeweight);
          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::TH2)->Integral(), 1.0);
          
          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::Prof)->GetBinContent(bin_x), largeweight*event[sct::GlauberObservable::Centrality]);
          EXPECT_EQ(var.get(mod, x_axis, sct::HistType::Weight)->GetBinContent(bin_x), largeweight);
          ...
          for (auto& order : {2, 4, 6}) {
            EXPECT_EQ(var.get(mod, x_axis, sct::HistType::MomentTH1, order)->Integral(), 0.0);
            EXPECT_EQ(var.get(mod, x_axis, sct::HistType::MomentProf, order)->Integral(), 
                      largeweight*pow(event[sct::GlauberObservable::Centrality], order));
            EXPECT_EQ(var.get(mod, x_axis, sct::HistType::MomentProf, order)->GetBinContent(bin_x), 
                      largeweight*pow(event[sct::GlauberObservable::Centrality], order));
            EXPECT_EQ(var.get(mod, x_axis, sct::HistType::Cumulant, order)->Integral(), 0.0);
          }
          ...
          for (auto& order : {2, 4, 6}) {
            EXPECT_EQ(var.get(mod, x_axis, sct::HistType::MomentProf, order)->Integral(), 0.0);
            EXPECT_EQ(var.get(mod, x_axis, sct::HistType::MomentTH1, order)->Integral(), 0.0);
            EXPECT_EQ(var.get(mod, x_axis, sct::HistType::Cumulant, order)->Integral(), 0.0);
          }
        }
...
double Get4thOrderCumulantError(double c2, double c4,
       double c2error, double c4error);
double Get6thOrderCumulantError(double c2, double c4, double c6,
       double c2error, double c4error, double c6error);
    ...
    double c2error, double c4error, double c6error)
{
  ...
  double error1   = (c4!=0.0 && c2!=0.0) ? 
    9.0 * abs(c4*c2) * sqrt(pow(c4error/c4, 2.0) + pow(c2error/c2, 2.0)) : 0.0;
  double error2   = 12.0 * 3.0 * c2*c2*c2error ;
  double error    = 0.25 * sqrt(c6error*c6error + error1*error1 + error2*error2) ;
