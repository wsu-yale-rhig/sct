...
enum class HistType { TH1, TH2, Prof, Weight, MomentTH1, MomentProf, Cumulant };
...
class SystematicVariable {
 public:
  ...
  // CAUTION: this has been copied from the old STAR maker without validation of
  // the correctness of the math or the algorithm. It has only been confirmed to
  // produce the same results as the old maker.
  ...
  // write histograms to specified TFile (will not change current TDirectory to
  // file)
  void write(TFile* file);

  // access to histograms - for testing and debugging
  TH1* get(GlauberMod mod, GlauberObservable x_axis, HistType tag,
           unsigned cumulant_order = 0);

 private:
  // returns a name for a histogram given the specifics
  string getHistogramName(GlauberMod mod, GlauberObservable x_axis,
                          HistType tag, unsigned cumulant_order = 0);
  ...
  // these functions have essentially been copied from the old maker for
  // completeness - the math has not been verified.
  double nthOrderCumulant(std::vector<double>& moments, unsigned order);
  double nthOrderCumulantError(std::vector<double>& moments,
                               std::vector<double>& errors, unsigned order);
  ...
  bool cumulant_flag_;
  std::set<unsigned> cumulant_order_ = {
      2, 4, 6};  // order of cumulants to calculate - odd cumulants will be zero

  // observables to calculate the SystematicVariable with respect to - e.g. for
  // x_ in x_axis_variables_ : plot(x_, y_)
  std::set<GlauberObservable> x_axis_variables_ = {
      GlauberObservable::B, GlauberObservable::Ncoll, GlauberObservable::Npart,
      GlauberObservable::Multiplicity};
