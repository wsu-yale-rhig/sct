...
void CentralityDef::setWeightParameters(const std::vector<double>& pars,
  ...
  weight_bound_ = 0;
  if (pars.size() != 7) {
    LOG(ERROR) << "reweighting require 7 parameters, but only " << pars.size()
...
bool CentralityDef::checkEvent(int runid, double refmult, double zdc,
                               double vz) {
  if (refmult < 0) return false;
  if (max_run_ > 0 && (runid < min_run_ || runid > max_run_)) return false;
  if (vz < min_vz_ || vz > max_vz_) return false;
  if (zdc < min_zdc_ || zdc > max_zdc_) return false;
  return true;
}
...
void CentralityDef::calculateCentrality(double refmult, double zdc, double vz) {
  ...
  if (zdc_par_.empty() || vz_par_.empty()) {
    LOG(ERROR) << "zdc and vz correction parameters must be set before "
