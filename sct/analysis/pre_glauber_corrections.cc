...
SCT_DEFINE_string(dataFile, "refmult.root", "path to root file containing refmult tree");
...
ProfileRefmultInBinsOfVz(TH3D* hist, std::string name_prefix, int nsplits = 4) {
  ...
  for (int i = 0; i < nsplits; ++i) {
    ...
    lumi_1d_vz.push_back((TProfile*) ((TH2D*)hist->Project3D("ZY"))->ProfileX(sct::MakeString(name, "_profile_", i).c_str()));
    ...
    lumi_1d_vz_string.push_back(sct::MakeString(vz_low, " < v_{z} < ", vz_high));
  }
...
ProjectRefMultInBinsOfVz(TH3D* hist, std::string name_prefix, int nsplits = 4) {
  ...
  for (int i = 0; i < nsplits; ++i) {
    ...
    refmult_1d_vz.push_back(hist->ProjectionZ(sct::MakeString(name, "_projection_", i).c_str(),
    ...
    double vz_low = FLAGS_vzMin + dvz * (low_bin - 1);
    double vz_high = FLAGS_vzMin + dvz * high_bin;
    refmult_1d_vz_string.push_back(sct::MakeString(vz_low, " < v_{z} < ", vz_high));
  }
...
ProjectRefMultInBinsOfZDC(TH3D* hist, std::string name_prefix, int nsplits = 4) {
  ...
  for (int i = 0; i < nsplits; ++i) {
    ...
    refmult_1d_lumi.push_back(hist->ProjectionZ(sct::MakeString(name, "_projection_", i).c_str(),
                              0, -1, low_bin, high_bin));
    ...
    refmult_1d_lumi_string.push_back(sct::MakeString(lumi_low, " < zdcX[kHz] < ", lumi_high));
  }
  return {refmult_1d_lumi, refmult_1d_lumi_string};
}

void RefmultQA(TH3D* h, std::string name, sct::histogramOpts hOpts_hist, sct::canvasOpts cOpts_hist,
               sct::histogramOpts hOpts_prof, sct::canvasOpts cOpts_prof, int bins = 4) {
  // print the uncorrected average refmult in bins of vz & bins of zdc
  std::pair<std::vector<TH1D*>, std::vector<std::string>>
  refmult_lumi_bins = ProjectRefMultInBinsOfZDC(h, name, bins);
  for (auto h : refmult_lumi_bins.first)
    h->Scale(1.0 / h->Integral());
  Overlay1D(refmult_lumi_bins.first, refmult_lumi_bins.second, hOpts_hist, cOpts_hist, FLAGS_outDir,
            name + "_ref_lumi_bin", "", "refmult", "fraction", "");
  
  std::pair<std::vector<TH1D*>, std::vector<std::string>>
  refmult_vz_bins = ProjectRefMultInBinsOfVz(h, name, bins);
  for (auto h : refmult_vz_bins.first)
    h->Scale(1.0 / h->Integral());
  Overlay1D(refmult_vz_bins.first, refmult_vz_bins.second, hOpts_hist, cOpts_hist, FLAGS_outDir,
            name + "_ref_vz_bin", "", "refmult", "fraction", "");
  
  // print out average refmult as a function of luminosity in bins of vz
  std::pair<std::vector<TProfile*>, std::vector<std::string>>
  lumi_1d_vz = ProfileRefmultInBinsOfVz(h, name, bins);
  lumi_1d_vz.first[0]->GetYaxis()->SetRangeUser(100, 200);
  Overlay1D(lumi_1d_vz.first, lumi_1d_vz.second, hOpts_prof, cOpts_prof, FLAGS_outDir,
            name + "_avg_ref_vs_lumi_vz", "", "ZDC Rate [kHz]", "<refmult>", "");
}

int main(int argc, char* argv[]) {
  
  // set help message and initialize logging and command line flags
  std::string usage = "Performs luminosity and Vz corrections to a refmult distribution tree. ";
  usage += "Input must have refmult, luminosity & vz branches - output is a second tree with corrected refmult.";
  ...
  TH3D* uncorr_lumi = new TH3D("uncorr_lumi", ";V_{z};luminosity [kHz];refmult", FLAGS_vzBins,
                               FLAGS_vzMin, FLAGS_vzMax, FLAGS_lumiBins, FLAGS_lumiMin, FLAGS_lumiMax,
                               (FLAGS_refmultMax - FLAGS_refmultMin), FLAGS_refmultMin, FLAGS_refmultMax);
  ...
  TProfile* uncorr_lumi_1d = (TProfile*) ((TH2D*)uncorr_lumi->Project3D("ZY"))->ProfileX("uncorr_profile");
  uncorr_lumi_1d->Rebin(4);
  // now fit with a first order polynomial for our luminosity correction
  TF1* luminosity_correction = new TF1("luminosity_correction", "[0]+[1]*x", FLAGS_lumiMin, FLAGS_lumiMax);
  ...
  sct::PrettyPrint1D(uncorr_lumi_1d, hOpts, cOptsNoLeg, "", FLAGS_outDir, "uncorr_avg_refmult_vs_lumi",
                     "", "ZDC Rate [kHz]", "<refmult>", "");
  ...
  reader.Restart();
  TH3D* corr_lumi = new TH3D("corr_lumi", ";V_{z};luminosity [kHz];refmult", FLAGS_vzBins,
                             FLAGS_vzMin, FLAGS_vzMax, FLAGS_lumiBins, FLAGS_lumiMin, FLAGS_lumiMax,
                             (FLAGS_refmultMax - FLAGS_refmultMin), FLAGS_refmultMin, FLAGS_refmultMax);
  ...
  // now generate a 1D profile, <refmult> vs luminosity, and fit to see if its approximately flat
  TProfile* corr_lumi_1d = (TProfile*) ((TH2D*)corr_lumi->Project3D("ZY"))->ProfileX("corr_profile");
  corr_lumi_1d->Rebin(4);
  corr_lumi_1d->GetYaxis()->SetRangeUser(100, 200);
  TF1* corr_ref_lumi_fit = new TF1("corr_ref_lumi_fit", "[0]+[1]*x", FLAGS_lumiMin, FLAGS_lumiMax);
  corr_lumi_1d->Fit(corr_ref_lumi_fit);
  
  sct::PrettyPrint1D(corr_lumi_1d, hOpts, cOptsNoLeg, "", FLAGS_outDir, "corr_avg_refmult_vs_lumi",
                     "", "ZDC Rate [kHz]", "<refmult>", "");
  ...
  TH1D* uncorr_vz_fit_h_param = new TH1D("uncorr_vz_fit_h_param", ";v_{Z};h", FLAGS_vzBins, FLAGS_vzMin, FLAGS_vzMax);

  for (int i = 1; i <= FLAGS_vzBins; ++i) {
    vz_uncorr_binned_refmult.push_back(corr_lumi->ProjectionZ(sct::MakeString("refmult_vz_bin_", i).c_str(),
                                                              i, i, 1, -1));
    vz_uncorr_binned_refmult[i-1]->Scale(1.0 / vz_uncorr_binned_refmult[i-1]->Integral());
    vz_uncorr_binned_refmult_fit.push_back(new TF1(sct::MakeString("refmult_vz_bin_fit_", i).c_str(),
                                                   "[0] + [0] * TMath::Erf([1]*(x-[2]))", 500, 700));
    vz_uncorr_binned_refmult_fit[i-1]->SetParameters(0.0001, -0.02, 480);
    vz_uncorr_binned_refmult[i-1]->Fit(vz_uncorr_binned_refmult_fit[i-1], "ME", "", 450, 800);
    uncorr_vz_fit_h_param->SetBinContent(i, vz_uncorr_binned_refmult_fit[i-1]->GetParameter(2));
    uncorr_vz_fit_h_param->SetBinError(i, vz_uncorr_binned_refmult_fit[i-1]->GetParError(2));
  }
  LOG(INFO) << "Fitting h parameter distribution with sixth order polynomial";
  TF1* vz_correction = new TF1("uncorr_vz_fit_param_fit", "pol6(0)", FLAGS_vzMin, FLAGS_vzMax);
  ...
  sct::PrettyPrint1D(uncorr_vz_fit_h_param, hOpts, cOptsNoLeg, "", FLAGS_outDir, "uncorr_vz_fit_h",
                     "", "v_{z} [cm]", "fit paramter h", "");
  
  // create luminosity & vz corrected refmult
  LOG(INFO) << "generating luminosity and Vz corrected refmult distribution";
  reader.Restart();
  TH3D* corr_lumi_vz = new TH3D("corr_lumi_vz", ";V_{z};luminosity [kHz];refmult", FLAGS_vzBins,
                                FLAGS_vzMin, FLAGS_vzMax, FLAGS_lumiBins, FLAGS_lumiMin, FLAGS_lumiMax,
                                (FLAGS_refmultMax - FLAGS_refmultMin), FLAGS_refmultMin, FLAGS_refmultMax);
  ...
  while(reader.Next()) {
    ...
    double corr_refmult = static_cast<double>(*refmult) * (zdc_norm * vz_norm) / (zdc_corr * vz_corr);

    corr_lumi_vz->Fill(*vz, lumikhz, corr_refmult);
  }
  
  RefmultQA(corr_lumi_vz, "full_corr", hOpts, cOptsLowerLegLogy, hOpts, cOpts, 4);
  ...
  TH1D* corr_vz_fit_h_param = new TH1D("corr_vz_fit_h_param", ";v_{Z};h", FLAGS_vzBins, FLAGS_vzMin, FLAGS_vzMax);
  
  for (int i = 1; i <= FLAGS_vzBins; ++i) {
    vz_corr_binned_refmult.push_back(corr_lumi_vz->ProjectionZ(sct::MakeString("refmult_vz_bin2_", i).c_str(),
                                                              i, i, 1, -1));
    vz_corr_binned_refmult[i-1]->Scale(1.0 / vz_corr_binned_refmult[i-1]->Integral());
    vz_corr_binned_refmult_fit.push_back(new TF1(sct::MakeString("refmult_vz_bin_fit2_", i).c_str(),
                                                   "[0] + [0] * TMath::Erf([1]*(x-[2]))", 500, 700));
    vz_corr_binned_refmult_fit[i-1]->SetParameters(0.0001, -0.02, 480);
    vz_corr_binned_refmult[i-1]->Fit(vz_corr_binned_refmult_fit[i-1], "ME", "", 450, 800);
    corr_vz_fit_h_param->SetBinContent(i, vz_corr_binned_refmult_fit[i-1]->GetParameter(2));
    corr_vz_fit_h_param->SetBinError(i, vz_corr_binned_refmult_fit[i-1]->GetParError(2));
  }
  sct::PrettyPrint1D(corr_vz_fit_h_param, hOpts, cOptsNoLeg, "", FLAGS_outDir, "corr_vz_fit_h",
                     "", "v_{z} [cm]", "fit paramter h", "");
  ...
  Overlay1D(uncorr_refmult, full_corr_refmult, "uncorrected refmult", "corrected refmult", hOpts,
            cOptsLowerLegLogy, FLAGS_outDir, "corrected_refmult", "", "refmult", "fraction");
