...
template <typename... Args> string MakeString(const Args &... args) {
  std::stringstream ss;
  MakeStringInternal(ss, args...);
  return string(ss.str());
}

template <> string MakeString(const string &s);
...
template <typename T>
std::vector<T> ParseArgStringToVec(string str, string sep = ",",
