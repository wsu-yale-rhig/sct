...
double StepFunction(double *x, double *par) {
  ...
  double distance =
      std::sqrt(std::pow(x_, 2) + std::pow(y_, 2) + std::pow(z_, 2));
...
double Gaussian(double *x, double *par) {
  ...
  double r = std::sqrt(std::pow(x_, 2) + std::pow(y_, 2) + std::pow(z_, 2));
...
double WoodsSaxonDeformed(double *x, double *par) {
  ...
  double Y40 = std::sqrt(1.0 / pi) * 3.0 / 16.0 *
               (35.0 * cos_theta4 - 30.0 * cos_theta2 + 3.0);
